...
import {
    AdminUsersApi, AdminUserResponse,
    AdminUserEditRequest, AdminRegisterRequest
...
export default function AdminHomePage() {
    ...
    const [addForm, setAddForm] = useState<AdminRegisterRequest>({
        firstName: '', lastName: '',
        email: '', password: '', roles: ['CLIENT']
    });

    const [editModalOpen, setEditModalOpen] = useState(false);
    const [userToEdit, setUserToEdit] = useState<AdminUserResponse | null>(null);
    const [editForm, setEditForm] = useState<AdminUserEditRequest>({
        firstName: '', lastName: '',
        email: '', enabled: true, roles: []
    });
                                ...
                                <TableBody>
                                    {users.map((user) => (
                                        <TableRow key={user.id} sx={{
                                            ...
                                            <TableCell sx={{textAlign: "center"}}>
                                                <IconButton onClick={() => handleOpenEdit(user)} sx={{
            ...
            <Dialog open={addModalOpen} onClose={() => setAddModalOpen(false)} fullWidth maxWidth="xs"
                ...
                <form onSubmit={handleConfirmAdd}>
                    ...
                    <DialogContent sx={{display: 'flex', flexDirection: 'column', gap: '16px', pt: '12px !important'}}>
                        ...
                        <TextField label="Email" type="email" required fullWidth value={addForm.email}
                                   onChange={(e) => setAddForm({...addForm, email: e.target.value})}/>
                        <TextField label="Password" type="password" required fullWidth value={addForm.password}
            ...
            <Dialog open={editModalOpen} onClose={() => setEditModalOpen(false)} fullWidth maxWidth="xs"
                ...
                <form onSubmit={handleConfirmEdit}>
                    ...
                    <DialogContent sx={{display: 'flex', flexDirection: 'column', gap: '16px', pt: '12px !important'}}>
                        ...
                        <TextField label="Email" type="email" required fullWidth value={editForm.email}
