...
export default function AdminMoviesPage() {
    ...
    const [addForm, setAddForm] = useState<AdminMovieRequest>({ title: "", description: "", duration: 0, imageUrl: "", available: true });

    const [editModalOpen, setEditModalOpen] = useState(false);
    const [movieToEdit, setMovieToEdit] = useState<AdminMovieResponse | null>(null);
    const [editForm, setEditForm] = useState<AdminMovieRequest>({ title: "", description: "", duration: 0, imageUrl: "", available: true });
    ...
    const handleOpenDelete = (e: React.MouseEvent, movie: AdminMovieResponse) => {
        e.stopPropagation(); // Blokuje przejście do seansów po kliknięciu w "Usuń"
    ...
    const handleOpenEdit = (e: React.MouseEvent, movie: AdminMovieResponse) => {
        e.stopPropagation(); // Blokuje przejście do seansów
    ...
    return (
        <Box sx={{ backgroundColor: colors.lightgrey, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
                                ...
                                <TableBody>
                                    {movies.map((movie) => (
                                        ...
                                        >
                                            <TableCell sx={{ textAlign: "center" }}>
                                                {movie.imageUrl ? (
                                                    <Box
                                                        ...
                                                        src={movie.imageUrl}
                                                        alt={movie.title}
                                                        sx={{ width: 50, height: 75, objectFit: 'cover', borderRadius: '4px', display: 'block', margin: '0 auto' }}
                                                    />
                                                ) : (
                                                    <Box sx={{ width: 50, height: 75, backgroundColor: '#eee', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '4px', margin: '0 auto' }}>
                                            ...
                                            <TableCell sx={{ textAlign: "center" }}>
                                                <IconButton onClick={(e) => handleOpenEdit(e, movie)} sx={{ color: colors.darkgrey, "&:hover": { color: colors.black } }}>
                                                    <Pencil size={18} />
                                                </IconButton>
                                                <IconButton onClick={(e) => handleOpenDelete(e, movie)} sx={{ color: colors.red || "#d32f2f", "&:hover": { color: "#b71c1c" } }}>
                                                    <Trash2 size={18} />
                                                </IconButton>
                                            </TableCell>
                                        </TableRow>
            ...
            {/* --- ADD MOVIE MODAL --- */}
            <Dialog open={addModalOpen} onClose={() => setAddModalOpen(false)} fullWidth maxWidth="sm" PaperProps={{ sx: { borderRadius: '12px', p: 1 } }}>
                <form onSubmit={handleConfirmAdd}>
                    ...
                    <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: '16px', pt: '12px !important' }}>
                        <TextField label="Movie Title" required fullWidth value={addForm.title} onChange={(e) => setAddForm({...addForm, title: e.target.value})} />
                        <TextField label="Description" required fullWidth multiline rows={4} value={addForm.description} onChange={(e) => setAddForm({...addForm, description: e.target.value})} />
                        <TextField label="Duration (minutes)" type="number" required fullWidth value={addForm.duration} onChange={(e) => setAddForm({...addForm, duration: e.target.value as any})} inputProps={{ min: 1 }} />
                        <TextField label="Poster Image URL" required fullWidth value={addForm.imageUrl} onChange={(e) => setAddForm({...addForm, imageUrl: e.target.value})} />
                        <FormControlLabel control={<Switch checked={addForm.available} onChange={(e) => setAddForm({...addForm, available: e.target.checked})} color="primary" />} label="Available to Users" />
                    </DialogContent>
            ...
            {/* --- EDIT MOVIE MODAL --- */}
            <Dialog open={editModalOpen} onClose={() => setEditModalOpen(false)} fullWidth maxWidth="sm" PaperProps={{ sx: { borderRadius: '12px', p: 1 } }}>
                <form onSubmit={handleConfirmEdit}>
                    <DialogTitle sx={{ fontWeight: '700', color: colors.black }}>Edit Movie</DialogTitle>
                    <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: '16px', pt: '12px !important' }}>
                        <TextField label="Movie Title" required fullWidth value={editForm.title} onChange={(e) => setEditForm({...editForm, title: e.target.value})} />
                        <TextField label="Description" required fullWidth multiline rows={4} value={editForm.description} onChange={(e) => setEditForm({...editForm, description: e.target.value})} />
                        <TextField label="Duration (minutes)" type="number" required fullWidth value={editForm.duration} onChange={(e) => setEditForm({...editForm, duration: e.target.value as any})} inputProps={{ min: 1 }} />
                        <TextField label="Poster Image URL" required fullWidth value={editForm.imageUrl} onChange={(e) => setEditForm({...editForm, imageUrl: e.target.value})} />
                        <FormControlLabel control={<Switch checked={editForm.available} onChange={(e) => setEditForm({...editForm, available: e.target.checked})} color="primary" />} label="Available to Users" />
                    </DialogContent>
                    <DialogActions sx={{ px: 3, pb: 2, mt: 1 }}>
                        <Box sx={{ display: 'flex', width: '100%', gap: '12px' }}>
