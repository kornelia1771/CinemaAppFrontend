...
export default function AdminScreeningsPage() {
    ...
    const [addForm, setAddForm] = useState<AdminScreeningRequest>({ movieId: Number(movieId), hallId: '' as any, screeningTime: '', ticketPrice: '' as any });

    const [editModalOpen, setEditModalOpen] = useState(false);
    const [screeningToEdit, setScreeningToEdit] = useState<ScreeningResponse | null>(null);
    const [editForm, setEditForm] = useState<AdminScreeningRequest>({ movieId: Number(movieId), hallId: '' as any, screeningTime: '', ticketPrice: '' as any });

    useEffect(() => {
        const token = localStorage.getItem("token");
        if (!token) { navigate("/login"); return; }
        if (!movieId) { navigate("/admin/movies"); return; }
            ...
            <Dialog open={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} fullWidth maxWidth="xs" PaperProps={{ sx: { borderRadius: '12px', p: 1 } }}>
                ...
                <DialogContent sx={{ pt: '12px !important' }}>
                    <Typography variant="body1" sx={{ color: colors.black }}>
                        Are you sure you want to delete the screening scheduled for <strong>{screeningToDelete ? formatDateTime(screeningToDelete.screeningTime) : ''}</strong>?
                    </Typography>
                </DialogContent>
                <DialogActions sx={{ px: 3, pb: 2, mt: 1 }}>
                    <Box sx={{ display: 'flex', width: '100%', gap: '12px' }}>
                        <Button onClick={() => setDeleteModalOpen(false)} variant="outlined" sx={{ flex: 1, borderRadius: '8px', textTransform: 'none', borderColor: colors.black, color: colors.black, fontWeight: '600' }}>Cancel</Button>
