...
export default function TicketsPage() {
    ...
    return (
        <Box sx={{backgroundColor: colors.lightgrey, minHeight: '100vh', display: 'flex', flexDirection: 'column'}}>
                                    ...
                                    <Box sx={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px'}}>
                                        <Box sx={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                                            ...
                                            <Box><Typography variant="caption" sx={{
                                                ...
                                                display: 'block'
                                            }}>Hall</Typography><Typography variant="body2"
                                                                            sx={{fontWeight: '600'}}>{ticket.hallName}</Typography></Box>
                                        </Box>
                                        <Box sx={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                                            ...
                                            <Box><Typography variant="caption" sx={{
                                                color: colors.darkgrey,
                                                display: 'block'
                                            }}>Seats</Typography><Typography variant="body2"
                                                                             sx={{fontWeight: '600'}}>{ticket.seatCounter}x</Typography></Box>
                                        </Box>
                                        <Box sx={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                                            ...
                                            <Box><Typography variant="caption" sx={{
                                                color: colors.darkgrey,
                                                display: 'block'
                                            }}>Date</Typography><Typography variant="body2"
                                                                            sx={{fontWeight: '600'}}>{formatOnlyDate(ticket.screeningTime)}</Typography></Box>
                                        </Box>
                                        <Box sx={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                                            <Ticket size={18} color={colors.darkgrey}/>
                                            <Box><Typography variant="caption"
                                                             sx={{color: colors.darkgrey, display: 'block'}}>Single
                                                Ticket Price</Typography><Typography variant="body2"
                                                                                     sx={{fontWeight: '700'}}>{Number(ticket.ticketPrice).toFixed(2)} PLN</Typography></Box>
                                        </Box>
                                        <Box sx={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                                            ...
                                            <Box><Typography variant="caption" sx={{
                                                color: colors.darkgrey,
                                                display: 'block'
                                            }}>Time</Typography><Typography variant="body2"
                                                                            sx={{fontWeight: '600'}}>{formatOnlyTime(ticket.screeningTime)}</Typography></Box>
                                        </Box>
                                        <Box sx={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                                            <Wallet size={18} color={colors.darkgrey}/>
                                            <Box><Typography variant="caption"
                                                             sx={{color: colors.darkgrey, display: 'block'}}>Total
                                                Price</Typography><Typography variant="body2"
                                                                              sx={{fontWeight: '700'}}>{Number(ticket.totalPrice).toFixed(2)} PLN</Typography></Box>
                                        </Box>
                                    </Box>
                                        ...
                                        <Box>
                                            {ticket.status === 'UNPAID' ? (
                                                <Typography variant="caption"
                                                            sx={{color: colors.darkgrey, fontStyle: 'italic'}}>
                                                    Please pay
                                                    by: {formatOnlyTime(ticket.dateDuePay)}, {formatOnlyDate(ticket.dateDuePay)} or
                ...
                <DialogContent sx={{pt: '12px !important'}}>
                    {ticketToCancel && (
                        <Typography variant="body1" sx={{color: colors.black}}>
                            ...
                            for <strong>{ticketToCancel.movieTitle}</strong> at <strong>{formatOnlyDate(ticketToCancel.screeningTime)}</strong> and <strong>{formatOnlyTime(ticketToCancel.screeningTime)}</strong> for <strong>{ticketToCancel.seatCounter}</strong> seats?
                        </Typography>
                    )}
                </DialogContent>
            ...
            <Snackbar open={apiError !== null} autoHideDuration={3000} onClose={() => setApiError(null)}
                      anchorOrigin={{vertical: 'bottom', horizontal: 'left'}}>
                <Alert onClose={() => setApiError(null)} severity="error"
                       sx={{width: '100%', borderRadius: '8px', fontWeight: '500'}} elevation={6}
                       variant="filled">{apiError}</Alert>
            </Snackbar>
            <Snackbar open={apiSuccess !== null} autoHideDuration={3000} onClose={() => setApiSuccess(null)}
                      anchorOrigin={{vertical: 'bottom', horizontal: 'left'}}>
                <Alert onClose={() => setApiSuccess(null)} severity="success"
                       sx={{width: '100%', borderRadius: '8px', fontWeight: '500'}} elevation={6}
                       variant="filled">{apiSuccess}</Alert>
            </Snackbar>
        </Box>
    );
}